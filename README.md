...
Cost is **read on-demand from Langfuse** at ticket-read time — there is
no periodic sync loop. The `with_cost()` helper in `runtime/deps.py`
populates `ticket.cost_usd` in-place via one of two functions from
`langfuse_client.py`:

| Call path | Used by | Behaviour |
|---|---|---|
| `session_cost()` | Ticket detail GET | Calls Langfuse API (live), 60 s in-memory cache |
| `session_cost_cached()` | `/tickets` board poll | Cache-only; never hits the network |

The board polls `/tickets` every 5 s, so the cache-only variant keeps
the board fast under load. Opening a ticket detail drawer triggers a
live fetch, which refreshes the cache for subsequent board polls.

There is no env var for a sync interval (no periodic sync loop). The cache TTL is
hard-coded at 60 s (`_COST_TTL_SECONDS` in `langfuse_client.py`).

### Graceful degradation

When Langfuse is unconfigured (`LANGFUSE_*` env vars absent) or
unreachable, cost reads are a **safe no-op**: `cost_usd` stays at `0.0`
