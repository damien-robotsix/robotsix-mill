...
| `pipeline.ci_fix_max_iterations` | `MILL_CI_FIX_MAX_ITERATIONS` | `5` | Single-repo ci-fix: max `wait_for_ci` push-and-recheck iterations the agent may run before BLOCK. The agent owns its fix→push→verify loop; this is its iteration budget. Set to 0 to disable the verify loop. Multiplies into the ci_fix stage ceiling — see `Settings.stage_timeout_for`. |
