...
| `core.limits.explore_timeout_seconds` | `MILL_EXPLORE_TIMEOUT_SECONDS` | `150.0` | Wall-clock timeout (seconds) for a single explore sub-agent call on the default (Claude) slot. Default 150 s. Minimum 1 s |
| `core.limits.explore_fallback_timeout_factor` | `MILL_EXPLORE_FALLBACK_TIMEOUT_FACTOR` | `3.0` | Multiplier applied to `explore_timeout_seconds` while the scout's level resolves to the OpenRouter fallback slot; a timed-out fallback attempt is not retried. Minimum 1 |
...
| `core.limits.stage_stall_window_seconds` | `MILL_STAGE_STALL_WINDOW_SECONDS` | `900` | At the soft stage deadline (`stage_timeout_seconds`), the run is killed only if it made no model/tool progress within this window; a slow-but-progressing run survives (bounded by the hard ceiling). `0` disables progress-awareness — kill at the soft deadline regardless of activity (legacy behavior). |
| `core.limits.stage_deadline_hard_multiplier` | `MILL_STAGE_DEADLINE_HARD_MULTIPLIER` | `3` | Unconditional hard-deadline ceiling as a multiple of the soft timeout (`stage_timeout_for(stage) * multiplier`); a run is killed here even while still making progress. |
...
| `gates.standards_gate_enabled` | `MILL_STANDARDS_GATE_ENABLED` | `true` | Pre-refine standards gate: park agent-spawned drafts whose goal violates an explicit robotsix-standards prohibition in `human_issue_approval` (never auto-closed to `done` — that is terminal and would silently discard an over-strict verdict; a human approves, rewrites, or closes it with a reason). Only repos that follow the fleet standards are gated (auto-detected from the `robotsix-` repo-id prefix; override per repo with `follows_standards` in repos.yaml); user-authored drafts are never gated |
...
| `sandbox.light_slots` | `MILL_SANDBOX_LIGHT_SLOTS` | `2` | Sandbox slots reserved for the explore scout's read-only commands (grep/find/read), a SEPARATE pool from `MILL_MAX_GLOBAL_CONCURRENCY`, so a scout grep never queues behind an implement test run holding a heavy slot (2026-09-07: 64 % of explore attempts were killed at the 90 s timeout while waiting for a slot). |
| `sandbox.light_op_timeout` | `MILL_SANDBOX_LIGHT_OP_TIMEOUT` | `60` | Per-command timeout (seconds) for light-lane sandbox commands; `0` falls back to `sandbox.op_timeout`. |
...
| `pipeline.ci_fix_max_total_attempts` | `MILL_CI_FIX_MAX_TOTAL_ATTEMPTS` | `2` | Hard per-ticket ceiling on the TOTAL number of ci-fix agent invocations for the same set of failing checks. The identical-failure gate resets whenever the agent pushes (its fingerprint includes the branch HEAD sha for rebase immunity), so a loop that keeps producing fresh-but-still-failing pushes can escape it and burn 6+ opus generations on one unfixable failure. This counter increments on every agent invocation and only resets when the failing check-name set changes (genuine progress), so it hard-caps the tail regardless of head churn. Set to 0 to disable. |
...
| `periodic.pin_bump_interval_seconds` | `MILL_PIN_BUMP_INTERVAL_SECONDS` | `86400` | Seconds between dependency pin-bump passes. Set to `0` to disable. |
...
| `periodic.infra_account_block_reescalate_seconds` | `MILL_INFRA_ACCOUNT_BLOCK_REESCALATE_SECONDS` | `86400` | Minimum seconds between fleet-wide operator escalations for the GitHub account/billing CI block (hosted runners refused) while it remains unresolved. The account-block recovery pass (run on the upstream-CI recovery loop) escalates once and auto-resumes parked tickets when hosted CI succeeds again. `0` = escalate every pass. |
...
| `periodic.diagnostic.interval_seconds` | `MILL_DIAGNOSTIC_INTERVAL_SECONDS` | `604800` | Seconds between diagnostic passes. Set to `0` to disable. Code default is `604800`; `config.example.json` overrides to `0` so the diagnostic pass is disabled by default in the example template. |
...
| `periodic.orphaned_pr_check.track_foreign_prs` | `MILL_ORPHANED_PR_TRACK_FOREIGN_PRS` | `false` | Also file tracking tickets for non-mill PRs (human / fleet-agent branches); never closes them |
| `periodic.orphaned_pr_check.foreign_ignore_branch_prefixes` | `MILL_ORPHANED_PR_FOREIGN_IGNORE_BRANCH_PREFIXES` | `["dependabot/", "renovate/", "release-please--", "pin-bump/", "bump/", "deps/"]` | Foreign PR head-branch prefixes never tracked — dependency, release and pin-bump PRs are owned by other automation (repo-hygiene / release periodics). Ignored PRs do not consume the per-pass caps. `[]` = track every foreign PR. |
