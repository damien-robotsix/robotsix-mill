...
2. **Explicitly-accepted advisory** — the step surfaces findings (CI
   ...
   [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "CI overview" section. An
...
|-------|------------|------------------|
| **jscpd** (duplicate code) | **Removed** — no longer in any workflow; [`.jscpd.json`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.jscpd.json) is preserved only as config for the on-demand `detect_duplication` agent tool. | `.jscpd.json` (config retained for the tool, not CI) |
| **mypy `--strict`** | **Advisory** — runs on every PR/push, emits an error-count CI annotation, `exit 0` so it never blocks (pre-existing ~700-error strict-mode backlog). Promotion: remove the advisory wrapper once the backlog is cleared. | Comment in [`ci.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/ci.yml) (Mypy step) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "CI overview" |
| **Bandit** | **Gate (with severity floor)** — `--severity-level medium` blocks on MEDIUM+ findings; LOW findings are an intentionally non-blocking backlog. | `[tool.bandit]` comment in [`pyproject.toml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/pyproject.toml) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) |
| **Trivy gate** (`release.yml` → reusable) | **Gate (narrowed)** — only CRITICAL, fixable (`ignore-unfixed: true`) CVEs fail the pipeline; HIGH/MEDIUM/LOW are high-volume/low-signal and do not gate. | Comments in [`release.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/release.yml) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "Trivy vulnerability scanning"; [`.trivyignore`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.trivyignore) per-CVE escape hatch |
| **Trivy SARIF upload** | **Advisory (observability-only)** — a separate Trivy run emits SARIF for all severities with no `exit-code` and uploads to the GitHub Security tab; never blocks. | Comments in [`release.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/release.yml) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "Trivy vulnerability scanning" |
| **hadolint** | **Advisory** — runs with `failure-threshold: warning`; warnings surface as CI annotations but do not block. Promotion: raise threshold to `error` once the Dockerfiles clear their known warnings. | [`.hadolint.yaml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.hadolint.yaml) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "CI overview" |
| **pip-audit** | **Gate (with justified suppressions)** — fails on known vulnerabilities; two CVEs are suppressed via `--ignore-vuln`, each justified inline: **PYSEC-2025-183** (pyjwt — key strength is the caller's responsibility, no fix published) and **MAL-2026-4750** (fastapi — the malicious dep ships only in `fastapi[standard]`, which mill never installs; advisory since withdrawn). | Inline comments in [`security-audit.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/security-audit.yml) |
| **pip-licenses** (license compliance) | **Gate (allowlist with per-package escape hatch)** — the `license-audit` job runs `pip-licenses --allow-only=…` over the installed `.[tracing]` tree and fails on **any** dependency whose license is not on the permissive allowlist (MIT, Apache-2.0, BSD, ISC, PSF, MPL-2.0, Unlicense), so copyleft (GPL/AGPL/LGPL) **and** unlicensed/`UNKNOWN` deps both fail; `--fail-on="GPL;AGPL;LGPL"` is kept as explicit belt-and-suspenders. Escape hatch: `--ignore-packages` suppresses the first-party MIT robotsix git deps that can report `UNKNOWN` metadata, plus rare third-party deps that are verifiably permissive but whose metadata format defeats the allowlist (e.g. `tiktoken`, which reports the full MIT license text instead of a short token), each justified inline. Policy lives in the workflow's CLI flags + comments (no separate `.licenserc`/`.scancode` file), matching the pip-audit `--ignore-vuln` / `[tool.bandit]` inline-policy convention. | Inline comments in [`security-audit.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/security-audit.yml) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "CI overview" |
| **commitizen** (commit-msg pre-commit hook) | **Gate (local only, not CI)** — the `commitizen` hook in `.pre-commit-config.yaml` validates every commit message against the Conventional Commits spec before it is accepted. Developers must run `.venv/bin/pre-commit install --hook-type commit-msg` once. Not a CI check; enforced at authoring time. Promotion path: add a CI step to validate commit messages on PRs if convention drift is observed. | Comment in [`.pre-commit-config.yaml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.pre-commit-config.yaml) + [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "Commit messages" |
...
- [ ] **If advisory, is the rationale written down in both places?** A
      ...
      [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) "CI overview" section.
...
- [ ] **Is it discoverable from [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md)?**
...
`uv.lock` bump PR ([`deps-bump.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/deps-bump.yml));
full rationale lives in [`dependencies.md`](../deps/dependencies.md).

## See also

- [`CONTRIBUTING.md`](https://github.com/damien-robotsix/robotsix-mill/blob/main/CONTRIBUTING.md) — "CI overview" section with
  per-check notes and the workflow table.
- [`.github/workflows/ci.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/ci.yml),
  [`.github/workflows/release.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/release.yml),
  [`.github/workflows/security-audit.yml`](https://github.com/damien-robotsix/robotsix-mill/blob/main/.github/workflows/security-audit.yml)
