...
| **Trivy gate** (`release.yml` → reusable) | **Gate (narrowed)** — only CRITICAL, fixable (`ignore-unfixed: true`) CVEs fail the pipeline; HIGH/MEDIUM/LOW are high-volume/low-signal and do not gate. | Comments in [`release.yml`](../.github/workflows/release.yml) + [`CONTRIBUTING.md`](../CONTRIBUTING.md) "Trivy vulnerability scanning"; [`.trivyignore`](../.trivyignore) per-CVE escape hatch |
| **Trivy SARIF upload** | **Advisory (observability-only)** — a separate Trivy run emits SARIF for all severities with no `exit-code` and uploads to the GitHub Security tab; never blocks. | Comments in [`release.yml`](../.github/workflows/release.yml) + [`CONTRIBUTING.md`](../CONTRIBUTING.md) "Trivy vulnerability scanning" |
