...
- In multi-repo mode, the audit agent runs independently for each
  ...
  project.
- Cross-repo ticket routing: findings that apply to a different repo
  (e.g. a shared standard deviation, a shared-library issue) include
  the target board_id in the draft body for the runner to route.
