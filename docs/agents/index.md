...
| CI prevention rules | `agent_definitions/periodic/ci_prevention_rules.yaml` | `agents/ci_prevention_rules.py` | level 2 | CLI (`ci-prevention-rules`), API (`POST /passes/ci_prevention_rules/run`), or daily via a `.robotsix-mill/periodic/ci_prevention_rules.yaml` presence file | Groups the board's recent `CI_FAILURE` events by failure bucket and rewrites a `## CI prevention rules (auto-maintained)` section at the top of the implement memory ledger; files no tickets |
...
- [docs/config/configuration.md](../config/configuration.md) — full env-var reference (maps every model var to its agent)
