...
| Cost-reconciliation | `agent_definitions/cost_reconciliation.yaml` | `agents/cost_reconciling.py` | `MILL_COST_RECONCILIATION_MODEL` | CLI, API (`POST /cost-reconciliation`), board button, or periodic (`MILL_COST_RECONCILIATION_PERIODIC`) | Detects OpenRouter ↔ Langfuse cost drift; invokes analysis agent and files draft when delta > $1 |
| Env-sync | (no agent — deterministic orchestrator in `env_sync_runner.py`) | `agents/env_syncing.py` | `MILL_ENV_SYNC_MODEL` | CLI (`env-sync`), API (`POST /env-sync`), board button, or periodic (`MILL_ENV_SYNC_PERIODIC`) | Scans for config ↔ .env ↔ docs drift; emits draft tickets for gaps |
