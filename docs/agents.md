...
| Cost-reconciliation | (no agent — deterministic pass in `cost_reconciliation_runner.py`) | — | — | CLI, API (`POST /cost-reconciliation`), board button, or periodic (`MILL_COST_RECONCILIATION_PERIODIC`) | Detects OpenRouter ↔ Langfuse cost drift; files a draft with raw figures when delta > $1 (no LLM call) |
