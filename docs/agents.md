...
| Cost-reconciliation | `agent_definitions/cost_reconciliation.yaml` | `agents/cost_reconciling.py` | `MILL_COST_RECONCILIATION_MODEL` | CLI, API (`POST /cost-reconciliation`), or periodic (`MILL_COST_RECONCILIATION_PERIODIC`) | Detects OpenRouter ↔ Langfuse cost drift; invokes analysis agent and files draft when delta > $1 |
