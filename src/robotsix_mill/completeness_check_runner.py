"""Completeness-check runner — orchestrates a single feature-
completeness inspection pass.

Mirrors the bc-check runner pattern: read memory, invoke agent,
write returned memory verbatim, collect emitted draft tickets.

Seam: tests monkeypatch ``run_completeness_check_agent`` from
...
log = logging.getLogger("robotsix_mill.completeness_check")
...
def run_completeness_check_pass(session_id: str, repo_config: RepoConfig | None = None) -> CompletenessCheckPassResult:
    """Execute one full feature-completeness inspection pass.

    Reads the memory ledger, invokes the completeness-check agent,
    writes the returned memory verbatim, and creates draft tickets for
    ...
    if repo_config is not None:
        ...
        memory_file = repo_data_dir / "completeness_check_memory.md"
        if repo_config.forge_remote_url:
            forge_remote_url = repo_config.forge_remote_url
            clone_dir = repo_data_dir / "completeness_check_workspace" / "repo"
    else:
    ...
    if forge_remote_url:
        ...
        cand = clone_dir or (settings.data_dir / "completeness_check_workspace" / "repo")
        if (cand / ".git").exists():
            repo_dir = cand
        else:
            try:
                ...
                log.warning(
                    "completeness_check clone failed, web/context-only: %s",
                    (e.stderr or "")[:200],
                )
