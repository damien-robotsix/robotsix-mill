            ...
            if result.next_action in ("proceed", "escalate"):
                ...
                if (
                    ...
                    try:
                        ...
                        _changed.update(
                            f.strip() for f in _d.splitlines() if f.strip()
                        )
                        _changed.update(
                            f.strip() for f in _u.splitlines() if f.strip()
                        )
                    except Exception:
                    ...
                    _substantive = [
                        f for f in _changed if not _changelog_only(f)
                    ]
                    if not _substantive:
                        _gaps = ""
                        try:
                        ...
                        cls._finalize(
                            ctx, ticket, repo_dir, branch, note,
                            ok=False,
