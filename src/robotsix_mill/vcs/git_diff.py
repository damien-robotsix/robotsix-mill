"""Git diff helpers — inspect, parse, and validate diffs in a workspace clone.

Extracted from ``git_ops.py`` to keep that module under a reasonable size.
All public names are re-exported through ``git_ops`` so no call site
changes are needed.
"""
...
def changed_source_files(
    ...
    """Return the list of added/modified/renamed source files between
    merge-base and *ref*.

    Uses ``git diff --name-only --diff-filter=AMR <merge-base>...<ref>``.
    ``R`` matters: git enables rename detection by default, so a file the
    branch *added* is re-classified as a rename the moment the target gains
    a similar file — which is exactly what a split/extract refactor looks
    like after a rebase. Filtering renames out made the post-rebase drop
    guard report such a file as silently dropped when it was present on the
    branch all along (live: auto-mail 945c, ``_archive_action_mixin.py``,
    detected ``R096`` against the sibling it was extracted from).
    ``--name-only`` prints the destination path for a rename, so the entry
    lands under the name the branch actually carries.
...
DEFAULT_REBASE_DROP_EXEMPT_PATHS = (
    ...
    ".secrets.baseline",
    # uv's resolved lockfile: regenerated by ``uv lock`` from pyproject plus
    # the whole dependency graph, so like the registries above its content is
    # a function of the tree rather than of one branch. A dependency a sibling
    # PR already landed leaves this branch's lock delta collapsed to nothing,
    # which the blob-equality excuse cannot clear (live: mill 0020, part 2/3
    # of a migration whose dependency part 1/3 had already merged).
    "uv.lock",
)
