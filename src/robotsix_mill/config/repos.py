...
class RepoConfig(BaseModel):
    ...
    # When True (the default), the merge stage may auto-merge green PRs for
    # this repo via the forge API without waiting for a human.  Must also
    # have the global ``auto_merge_enabled`` set True and the global
    # kill-switch ``auto_merge_kill_switch`` set False.
    #
    # Defaults to True — repos.yaml is machine-generated by the runtime
    # registration path, which writes only ``board_id`` /
    # ``forge_remote_url``.  An opt-IN default therefore left every repo
    # permanently un-mergeable: every green, review-approved PR bounced
    # back to HUMAN_MR_APPROVAL forever and the board filled with tickets
    # no automated stage could ever advance.  Auto-merge stays gated by
    # the global switch, the global kill-switch, the review gate, the
    # trusted-bot-author check and the sensitive-path globs, so an
    # opt-OUT default here does not widen what can be merged — it only
    # stops the per-repo flag from being an unsatisfiable gate.  Set
    # ``auto_merge_enabled: false`` on a repo entry to opt that repo out.
...
def load_repos_config(config_file: str | None = None) -> ReposRegistry:
    ...
    for repo_id, repo_data in repos_mapping.items():
        ...
        repos[repo_id] = RepoConfig(
            ...
            # Previously omitted entirely: the field was declared on
            # RepoConfig but never read here, so an operator setting
            # ``auto_merge_enabled`` in repos.yaml was silently dropped and
            # every repo kept the field default. Auto-merge was therefore
            # unreachable fleet-wide regardless of configuration.
