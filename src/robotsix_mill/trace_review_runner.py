...
def _find_prior_matching_ticket(
    ...
    """Look up recent ``source=TRACE_REVIEW`` tickets on *board_id* and
    return the first one that matches the given fix signal.

    A candidate matches when, within the configured recency window:
    - any path in *target_files* appears verbatim in the candidate's
      description body, OR
    - the normalized symptom fingerprint (first ~60 normalized chars)
      appears in the candidate's normalized title.

    Candidates in CLOSED state that were never DONE (declined drafts)
    are EXCLUDED — declining is not a fix, so a new occurrence
    deserves a fresh draft.

    Returns ``None`` when no match is found.
    """
    try:
        cutoff = now - timedelta(days=settings.trace_review_dedup_lookback_days)
        candidates = service.recent_proposals_for(
            SourceKind.TRACE_REVIEW,
            limit=200,
        )
        fingerprint = _normalize(symptom)[:60]
        for ticket in candidates:
            ...
            # Symptom fingerprint check (normalized title).
            if fingerprint and fingerprint in _normalize(ticket.title):
                return ticket
        return None
    except Exception:  # noqa: BLE001 — best-effort dedup
        log.exception("trace-review: _find_prior_matching_ticket failed")
        return None
...
def run_trace_review_pass(
    ...
    for trace in traces:
        ...
        for finding in result.findings:
            ...
            prior = _find_prior_matching_ticket(
                service,
                target_board_id,
