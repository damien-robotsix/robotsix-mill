"""Pin-bump scheduled runner — dry-run detection + reporting only.
...
dependency graph (topological order + current SHAs), and logs the
snapshot.  No LLM call — pure graph computation.

PR creation (SHA-latest resolution → ``pyproject.toml`` edit →
``uv lock`` → cross-repo PR) is tracked separately and is explicitly
out of scope here.
"""
...
from robotsix_mill.config.repos import RepoConfig, get_repos_config, target_branch_for
