...
def run_bc_check_pass(session_id: str, repo_config: RepoConfig | None = None) -> BcCheckPassResult:
    ...
    """
    settings = Settings()
    memory_file = settings.memory_file_for('bc_check', repo_config.board_id if repo_config else '')
    ...
    if repo_config is not None:
        ...
        memory_file = repo_data_dir / "bc_check_memory.md"
        if repo_config.forge_remote_url:
            forge_remote_url = repo_config.forge_remote_url
            clone_dir = repo_data_dir / "bc_check_workspace" / "repo"
    else:
    ...
    if forge_remote_url:
        ...
        cand = clone_dir or (settings.data_dir / "bc_check_workspace" / "repo")
        if (cand / ".git").exists():
            repo_dir = cand
        else:
            try:
