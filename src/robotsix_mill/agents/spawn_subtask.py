...
async def run_spawn_subtask(
    *,
    ...
    system_prompt = (
        ...
        f"- For file editing, prefer `edit_file` over `sed`/`awk` for "
        f"anything beyond simple greps. `edit_file` is synchronous and "
        f"cheap; a sed-based approach runs a shell subprocess that can "
        f"take minutes for large files.\n"
