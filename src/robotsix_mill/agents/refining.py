...
_SYSPROMPT_PATH = agent_definitions_dir() / "refine.yaml"
...
def _build_refine_overrides(
    ...
    The model comes from the definition's ``level`` (refine is level 2
    → Claude Opus).
    """
...
def run_refine_agent(
    *,
    ...
    # Right-size the level-2 Claude model (subscription transport unchanged).
    # Only the Claude-SDK workhorse branch consumes the alias.
    resolved_level = (
        refine_level
        if refine_level is not None
        else resolve_agent_level(settings, definition)
    )
    if resolved_level == 2 and refine_model is not None:
