...
- **Model tier rebalance:** move non-critical pipeline stages off `claudeSDK-opus` (level 3) to reduce weekly Claude subscription token pressure. `epic_breakdown` moves from level 3 (opus) to level 4 (fable-5) — fable-5 handles structured JSON output reliably at lower cost. `pipeline/meta_triage` moves from level 3 (opus) to level 2 (DeepSeek pro) — repo-triage classification doesn't need Claude reasoning. `refine` remains at level 3 (opus) as the only stage that genuinely needs it. Level 4 is now a valid agent-definition tier, documented in the schema.
