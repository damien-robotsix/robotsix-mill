...
- Sandbox: add ``hatchling`` and the four git-sourced packages (``robotsix-llmio``, ``robotsix-board``, ``robotsix-config``, ``robotsix-modules``) to the sandbox Docker image so the per-invocation ``pip install .`` / ``uv sync`` never needs the egress proxy for them.  Fix misleading ``WARNING: uv not found`` message in ``_maybe_install_prefix`` — when ``uv`` is found but ``uv sync`` fails, the shell now reports ``uv sync failed`` instead.
- Make ``install_project`` (the sandbox project-install prefix) best-effort: when the egress proxy is unreachable, the install step is skipped rather than blocking the test command.  The sandbox image already pre-installs test-gate binaries (pytest, pytest-asyncio, pytest-cov), so a proxy outage no longer prevents the test suite from running.
