...
- Extract ``_resolve_current_ticket()`` helper in ``_ticket_context.py`` to deduplicate the lazy-import + guard pattern that was repeated identically across ``reply_thread``, ``list_threads``, ``close_thread``, ``post_comment``, and ``ask_user``.
