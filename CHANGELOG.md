...
- Add operator-facing `Field(description=…)` to every settings, secrets, and repos config field so the central-deploy Configure UI renders help bubbles instead of bare key paths. Regenerate `config/config.schema.json` with all descriptions. Move implementation notes out of the `Secrets` class docstring so the published schema description is operator-facing. Include `ReposRegistry` in the published schema under a `repos` key.
