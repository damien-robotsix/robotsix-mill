...
- Add missing `deps-bump.yml` workflow (weekly `uv lock --upgrade` → automated PR) matching the documented pin+bump mechanism for `robotsix-*` git dependencies. Fix stale `docker-publish.yml` → `release.yml` / GHCR references in `deployment.md` and `ci-policy.md`.
