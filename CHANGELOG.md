...
- Move CHANGELOG wording hygiene from review gate to merge-stage advisory lint: review stage no longer flags wording/overlap/labeling nits (only structural corruption). A new ``_lint_changelog`` in the merge stage detects empty bullets and overlapping entries (shared code identifiers), and ``_changelog_warnings_for_ticket`` adds a missing-entry advisory. Warnings surface in the merge-info drawer panel (amber for ``warn``, blue for ``info``) and are logged at WARNING level during ``_handle_human_mr_approval`` — advisory, never blocking. (mill: agent-yaml-schema.md: update stale `tester` reference to `run_tests` (20260717T040222Z-agent-yaml-schema-md-update-stale-tester-3125))
- Refine fast-path now checks for empty/near-empty descriptions and oversized scope before auto-approving. Tickets with empty or trivially short (< 50 chars) descriptions, or those mentioning > 7 distinct file paths, fall through to full refine instead of short-circuiting. Degenerate refine output (empty/placeholder specs) now routes to BLOCKED instead of the approval gate. All fast-path outcomes record which checks ran and their results for auditability.
- **Merge stage: green-but-behind PRs now rebase instead of waiting forever.** Under a strict up-to-date branch policy (GitHub `strict_required_status_checks_policy`), a PR with green CI whose branch is behind the target reports `mergeable_state="behind"` indefinitely — no re-poll changes it, and every auto-merge of a sibling PR strands the survivors further behind. `IMPLEMENT_COMPLETE` now routes `conclusion=success` + `mergeable_state=behind` to `REBASING` so the rebase agent catches the branch up (surfacing any semantic conflict) and CI reruns on the new head; `WAITING_AUTO_MERGE` and `HUMAN_MR_APPROVAL` fall back to `IMPLEMENT_COMPLETE` in the same situation so the gate check dispatches the rebase.
...
- Add `close_threads(comment_ids: list[int])` batch tool to close multiple review threads in a single API round-trip, replacing repeated `close_thread` calls in the refine sendback prompt to reduce Opus latency and cost.
...
- Implement stage stall hardening refinements: (1) `reap_orphan_sandboxes()` now takes `max_age_seconds=2*sandbox_op_timeout` in the timeout handler so mid-process reaping won't kill other tickets' legitimate sandboxes. (2) Renamed `implement_progress_timeout` → `implement_pass_timeout` with accurate flat-wall-clock description. (3) Moved `implement_pass_timeout` to the stage-runner level in `processing.py`, wrapping the full stage (scope-triage, rebase, sandbox setup/teardown, agent call) and routing implement timeouts through transient retry instead of hard-blocking. (4) Added periodic progress heartbeat (asyncio task logging every `max(15, timeout//6)` seconds) and distinguishable STALL logging with `error.subtype = "stall"` OTel attribute when the implement stage deadline fires.
- Implement stage timeout hardening: (1) Added `sandbox_op_timeout` (default 300s) so per-docker-op caps are 5 min instead of 1800s. (2) Fixed `_call_with_timeout` to use `shutdown(wait=False)` so watchdog TimeoutError propagates immediately instead of blocking on hung threads. (3) Added inner `TimeoutError` OTel span classification + `reap_orphan_sandboxes()` call in `processing.py` so watchdog kills are reflected in trace latency with no orphaned children. (4) Classified `SandboxError` (docker 'unexpected EOF') as transient in `_check_one_transient` for retry-with-backoff. (5) Added `implement_pass_timeout` (default 300s) as a flat wall-clock cap per implement agent pass (applied at stage-runner level, wrapping full stage including scope-triage, rebase, sandbox setup/teardown). (6) Added regression tests.
...
- Add pre-flight heuristic to the ``ask_web_knowledge`` sub-agent prompt encouraging targeted searches against known canonical sources before launching open-ended research loops, reducing wasted web-search budget on questions answerable from a single known reference file.
...
- Add `retry_on_db_full` context manager in `core/db.py` that catches disk-full/locked SQLite errors, runs an emergency VACUUM, retries once, and raises `DiskFullError` on failure. Wrap all service-mixin commit/flush sites and `persist_memory_db` with it. Expose disk usage on `/health` for proactive detection. (mill: Add retry/degradation handling for sqlite3 disk-full OperationalError across DB write paths (20260712T120728Z-add-retry-degradation-handling-for-sqlit-3188))
