...
- `UnexpectedModelBehavior` (pydantic-ai) is now treated as a transient error: agent runs retry with exponential backoff before degrading or falling back to a weaker model. This absorbs transient upstream API gateway glitches (null/malformed responses from OpenRouter) without losing the agent run.
