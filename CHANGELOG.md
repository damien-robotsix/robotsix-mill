...
- **runners**: consolidate 15 structurally-identical periodic-pass runner
  ...
  `periodic_runner`.  Each pass key still has a module-level callable
  (e.g. `run_audit_pass`, `run_survey_pass`) generated by a factory, so
  all existing CLI and route dispatch strings continue to resolve.
...
- **ci**: add `commitizen` commit-msg pre-commit hook to enforce
  ...
  pipeline that triggers PyPI publishing via the shared
  `python-release.yml` reusable workflow on GitHub Release publish.
  Set `build_command = "uv build"` so distributions are built and
...
- **refine**: tighten the refine agent's system prompt (~18% reduction) and add
  a fast-path instruction for mechanically simple drafts to reduce per-call
  token overhead; add a re-refinement deduplication rule to the reviewer
  sendback prompt to prevent redundant codebase re-exploration.
