...
- Deduplicate terminal-state constant: moved shared `_TERMINAL_STATES` to `src/robotsix_mill/core/states.py`, replacing the local `_LIST_TERMINAL_STATES` in `_tickets.py` and `_BOARD_LIST_TERMINAL` in `_board.py`.
