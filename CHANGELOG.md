...
- The explore sub-agent's SANDBOX GUARD now warns against trusting absolute paths from caller questions (check repo root via `git remote -v` or `pyproject.toml` first) and instructs the agent to fall back to repo CWD (`.`) on sandbox-escape errors instead of retrying the same absolute path.
