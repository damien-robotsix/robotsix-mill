...
- Rework the audit periodic agent into a **frontier orchestrator** (level 3) that decomposes the repository into 3–7 subparts, fans out sub-agents via `explore`/`parallel_explore`, and synthesises findings. Includes shared run memory protocol (`artifacts/audit-run-<run-id>/memory.md`), two-lens auditing per subpart (general health + standards conformance), and cross-repo ticket routing support.
- **Audit workflow v3**: reworked the audit periodic agent into a frontier orchestrator at level 3 with sub-agent fan-out, shared run memory (`artifacts/audit-run-<run-id>/memory.md`), two-lens mandate per subpart (health/maintainability + standards conformance), and cross-repo ticket routing. The orchestrator decomposes the repo into 3–7 subparts, spawns dedicated sub-agents via `explore`/`parallel_explore`, and synthesises findings into draft tickets.
