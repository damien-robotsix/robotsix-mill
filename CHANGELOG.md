...
- Fix config-sync gap: update ``coordinator_timeout_seconds`` Pydantic model default from ``900`` to ``600`` in ``_settings_core.py`` to match the ``config/config.example.json`` value (``600``) and ``docs/configuration.md`` documented default (``600``). The ``ge=60`` validator and ``MILL_COORDINATOR_TIMEOUT_SECONDS`` alias are unchanged.
