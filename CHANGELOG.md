...
- Remove `board_id` field from `RepoConfig` — the board identifier is now the `repo_id` (1:1 mapping). Legacy configs with `board_id == repo_id` are accepted with a warning; `board_id != repo_id` raises a config error. The add-repo API/UI now takes a single `repo_id` field. The synthetic `meta` board is preserved as the sole exception.
