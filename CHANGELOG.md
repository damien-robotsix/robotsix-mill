...
- Default `allow_runtime_repo_registration` to `true` so the add-repo UI works out of the box. The flag remains as an explicit opt-out for lock-down deployments that want config-only repos. Also gates `POST /tickets/ingest` and `POST /tickets` for auto-registered repos.
