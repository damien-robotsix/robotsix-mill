...
- [ ] Commit subjects are [conventional](#commit-messages); no hand edits to `CHANGELOG.md`

## Changelog

`CHANGELOG.md` is generated by release-please from conventional commit
subjects (see `release-please-config.json`). Do not edit it by hand and
do not add changelog fragment files — the commit subject / PR title
(`feat:`, `fix:`, `chore:`, `docs:`, `refactor:`, `test:`, `ci:`) is the
changelog entry.
