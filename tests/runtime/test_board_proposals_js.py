"""Drive the board.js Proposals-panel JS tests from pytest.

The Proposals panel lives in ``src/robotsix_mill/runtime/static/board.js``
— a flat browser script with no Python seam and no JS test runner in the
repo. ``board_proposals_harness.mjs`` loads the real script into Node's
...
def test_board_proposals_panel_js() -> None:
    ...
    if node is None:
        pytest.skip("node not on PATH — skipping board.js Proposals harness")
    ...
    assert result.returncode == 0, (
        "board.js Proposals harness failed "
