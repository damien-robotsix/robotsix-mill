...
from robotsix_mill.agents.runners.changelog_autofill_runner import (
    _format_entry,
...
class TestRunChangelogAutofillPass:
    def test_no_repo_config(self):
        ...
        with patch(
            "robotsix_mill.agents.runners.changelog_autofill_runner.get_forge"
        ) as mock_gf:
    ...
    def test_skip_skip_changelog_label(self):
        ...
        with (
            ...
            patch(
                "robotsix_mill.agents.runners.changelog_autofill_runner.github_push_token",
                return_value="tok",
            ),
            patch("robotsix_mill.agents.runners.changelog_autofill_runner.Settings"),
            patch(
                "robotsix_mill.agents.runners.changelog_autofill_runner.git_ops"
            ) as mock_git,
        ):
            run_changelog_autofill_pass(
                session_id="s1",
    ...
    def test_commits_entry_for_bot_pr(self):
        ...
        with (
            ...
            patch(
                "robotsix_mill.agents.runners.changelog_autofill_runner._insert_changelog_entry"
            ) as mock_insert,
        ):
            run_changelog_autofill_pass(
                session_id="s1",
