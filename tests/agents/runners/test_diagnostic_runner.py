...
def test_config_example_json_has_diagnostic_settings():
    ...
    config_path = Path(__file__).resolve().parents[3] / "config" / "config.example.json"
    with open(config_path) as fh:
        data = json.load(fh)
    settings = data["settings"]
    assert settings["diagnostic_interval_seconds"] == 0
...
def _fake_worker(enabled):
    settings = SimpleNamespace(
        diagnostic_interval_seconds=604800 if enabled else 0,
    )
