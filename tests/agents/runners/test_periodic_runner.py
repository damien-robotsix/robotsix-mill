...
def test_clone_token_returns_token_on_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.github_token", fake_github_token
    )
...
def test_clone_token_returns_none_on_http_error(monkeypatch):
    """_clone_token returns None when github_token raises httpx.HTTPStatusError
    (e.g. 404 from GET /repos/{owner}/{repo}/installation — GitHub App
    not installed on the repo)."""
...
def test_clone_token_logs_warning_on_app_not_installed(monkeypatch, caplog):
    ...
    def fake_github_token(settings, repo_config=None):
        raise rga.TokenMintError("GitHub App not installed on owner/repo")
    ...
    result = _clone_token(Settings(), _test_repo_config())
    assert result is None
    assert "Clone token unavailable" in caplog.text
...
def test_clone_token_uses_gitlab_token_for_gitlab_repo(monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.gitlab_token", fake_gitlab_token
    )
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.github_token", boom_github_token
    )
...
def test_clone_token_uses_github_token_for_github_repo(monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.gitlab_token", boom_gitlab_token
    )
...
def test_clone_token_gitlab_missing_token_returns_none(monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.gitlab_token",
        raising_gitlab_token,
    )
...
def test_forge_token_returns_secret(monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.get_secrets", lambda: fake_secrets
    )
...
def test_run_periodic_pass_clone_failure_sets_repo_dir_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.run_agent_pass",
        fake_run_agent_pass,
    )
...
def test_run_periodic_pass_uses_forge_token_when_clone_token_fn_none(
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.get_secrets", fake_get_secrets
    )
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.run_agent_pass",
        lambda **kw: _fake_agent_pass_result(),
...
def test_periodic_pass_configs_requires_repo():
    """Only module_curator, test_gap, state_sync, and
    frontend_sync set requires_repo=True (all need a
    clone to inspect files). All other registry entries keep the default False."""
