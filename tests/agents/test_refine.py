...
# ---------------------------------------------------------------------------
# Already-done verifier tests (acceptance criteria 1–6)
# ---------------------------------------------------------------------------
...
def test_jaccard_match_alone_does_not_yield_no_change_needed(
    ...
    monkeypatch.setattr(
        refining,
        "verify_already_done",
        lambda **_: refining.AlreadyDoneResult(
            already_done=False,
...
def test_no_repo_dir_never_short_circuits_on_memory_alone(
    ...
    def spy_verifier(**kwargs):
        ...
        return refining.AlreadyDoneResult(
            already_done=True, rationale="should not be used"
        )
...
def test_verify_already_done_tools_when_repo_dir_provided(settings, tmp_path):
    """Acceptance criterion 3 (tool wiring): when ``repo_dir`` is given,
    ``verify_already_done`` wires ``explore``, ``read_file``, and
    ``run_command`` tools, and is bounded by
    ``settings.already_done_request_limit``."""

    # We test the tool wiring by calling the verifier with a real repo_dir
    # and checking that the agent is built.  To avoid needing a real LLM,
    # we stub the inner parts.  However, the tool names are set on the
    # agent before the LLM call, so we can inspect them.

    # The simplest verification: ensure the function calls
    # build_agent_from_definition with tools containing the expected names.
    import robotsix_mill.agents.base as base_module
    import robotsix_mill.agents.retry as retry_module

    captured_tool_names: list[set] = []

    orig_build = base_module.build_agent_from_definition

    def spy_build(settings_arg, definition, *, tools=None, **overrides):
        if tools:
            captured_tool_names.append({t.__name__ for t in tools})
        return orig_build(settings_arg, definition, tools=tools, **overrides)

    monkeypatch = pytest.MonkeyPatch()
    monkeypatch.setattr(base_module, "build_agent_from_definition", spy_build)
    monkeypatch.setattr(
        retry_module,
        "run_agent",
        lambda agent, make_run, *, what="model call", sleep=None: make_run(agent),
    ...
    try:
        _result = refining.verify_already_done(
            settings=settings,
            title="test",
            draft="test draft",
            candidate_rationale="prior rationale",
            repo_dir=repo_dir,
        )
    except Exception:
        # Expected — the LLM call will fail without a real API key.
        # But build_agent_from_definition was called first, so we
        # captured the tool names.
        pass
    finally:
        monkeypatch.undo()

    # At least one call to build_agent_from_definition was made with
    # tools that include explore, read_file, and run_command.
    assert len(captured_tool_names) >= 1
    tool_names = captured_tool_names[0]
    assert "explore" in tool_names
    assert "read_file" in tool_names
    assert "run_command" in tool_names
    # write_file / edit_file / delete_file must NOT be present.
...
def test_verifier_not_called_without_memory_match(settings, tmp_path, monkeypatch):
    ...
    def spy_verifier(**kwargs):
        verifier_called.append(True)
        return refining.AlreadyDoneResult(already_done=True, rationale="x")
