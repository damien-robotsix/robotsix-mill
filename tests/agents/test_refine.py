...
# ---------------------------------------------------------------------------
# request_new_repo tool (meta refine → new-repo scaffold marker)
# ---------------------------------------------------------------------------


def test_request_new_repo_tool_records():
    """The tool records a normalized request into its sink and returns a
    confirmation (no marker formatting is left to the model)."""
    from robotsix_mill.agents.refining import make_request_new_repo_tool

    sink: dict = {}
    tool = make_request_new_repo_tool(sink)
    msg = tool(
        name="  robotsix-foo ",
        owner="org",
        private=True,
    ...
    assert "robotsix-foo" in msg


def test_run_refine_agent_injects_new_repo_marker(tmp_path, monkeypatch):
    """When the meta refine agent calls request_new_repo, the refined spec
    carries the canonical new-repo scaffold marker — generated
    deterministically, parseable by parse_new_repo_params, single-scoped."""
    ...
    from robotsix_mill.repo_scaffold import parse_new_repo_params
    ...
    class _FakeResult:
        output = RefineResult(spec_markdown="Create the new repo.")
    ...
    class _FakeHandle:
        def run_sync(self, *a, **k):
            for t in captured.get("tools", []):
                if getattr(t, "__name__", "") == "request_new_repo":
                    t(name="robotsix-foo", language="python", description="A lib")
            return _FakeResult()

        def close(self):
            pass

    def _fake_build(settings, definition, *, tools=None, **kw):
        captured["tools"] = tools or []
        return _FakeHandle()

    monkeypatch.setattr(_base, "build_agent_from_definition", _fake_build)

    out = run_refine_agent(
        settings=settings,
        title="Extract foo into a standalone lib",
        draft="Extract the foo helpers into robotsix-foo.",
        board_id="meta",
    )
    names = [getattr(t, "__name__", "") for t in captured["tools"]]
    assert "request_new_repo" in names  # tool offered on the meta board
    assert out.spec_markdown and "meta-extraction-kind: new-repo" in out.spec_markdown
    assert parse_new_repo_params(out.spec_markdown)["name"] == "robotsix-foo"
    assert out.split is False


def test_run_refine_agent_no_new_repo_tool_off_meta(tmp_path, monkeypatch):
    """The request_new_repo tool is meta-board-only — not offered elsewhere."""
    import robotsix_mill.config as _cfg
    from robotsix_mill.agents import base as _base
    from robotsix_mill.agents.refining import run_refine_agent
    from robotsix_mill.config import Secrets
    ...
    run_refine_agent(settings=settings, title="x", draft="y", board_id="robotsix-mill")
