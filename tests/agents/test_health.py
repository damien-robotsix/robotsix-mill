...
def test_run_health_pass_creates_draft_tickets(tmp_path, monkeypatch):
    ...
    health_tickets = [
        t for t in tickets if t.source == "health" and t.kind == TicketKind.TASK
    ]
...
def test_health_cli_command(capsys, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.run_health_pass", mock_run
    )
...
def test_post_health_check_returns_202(tmp_path, monkeypatch, repos_registry):
    ...
    monkeypatch.setattr(
        "robotsix_mill.agents.runners.periodic_runner.run_health_pass", slow_run
    )
...
def test_health_draft_blocked_when_test_dir_exists(tmp_path, monkeypatch):
    ...
    from robotsix_mill.agents.runners.pass_runner import run_agent_pass
