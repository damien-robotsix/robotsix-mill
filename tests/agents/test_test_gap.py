...
from robotsix_mill.test_gap_runner import run_test_gap_pass, TestGapPassResult
...
def test_test_gap_system_prompt_covers_key_dimensions():
    ...
    for kw in (
        "zero dedicated", "indirect", "i/o surface",
...
def test_run_test_gap_pass_empty_memory(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.test_gap_runner.Settings", lambda: settings
    )
...
def test_run_test_gap_pass_creates_draft_tickets(tmp_path, monkeypatch):
    ...
    def mock_agent(**kwargs):
        return test_gap_agent.TestGapResult(
            updated_memory="# Memory\n",
            draft_titles=["test gap: add unit tests for agents/refining.py",
...
def test_test_gap_cli_command(capsys, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.test_gap_runner.run_test_gap_pass", mock_run
    )
...
def test_test_gap_cli_json_output(capsys, tmp_path, monkeypatch):
    ...
    assert data["tickets_created"] == [{"id": "123", "title": "test gap: add unit tests for X"}]
...
def test_run_test_gap_pass_opens_langfuse_session(tmp_path, monkeypatch):
    """Each test-gap run wraps the agent in a Langfuse session span with a
    unique per-run id."""
    import contextlib
    ...
    res = run_test_gap_pass()

    assert res.session_id.startswith("test-gap-")
    assert seen["session_id"] == res.session_id
    assert seen["stage"] == "test-gap"
...
def test_test_gap_session_ids_are_unique_per_run(tmp_path, monkeypatch):
    settings = _make_settings(tmp_path)
    monkeypatch.setattr(
        test_gap_agent, "run_test_gap_agent",
    ...
    a = run_test_gap_pass().session_id
    b = run_test_gap_pass().session_id
    assert a != b and a.startswith("test-gap-") and b.startswith("test-gap-")
...
def test_run_test_gap_pass_no_forge_is_repo_dir_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        test_gap_agent.TestGapResult(updated_memory="m", draft_titles=[],
...
def test_post_test_gap_returns_202(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robotsix_mill.test_gap_runner.run_test_gap_pass", slow_run
    )
...
def test_post_test_gap_runs_in_background(tmp_path, monkeypatch):
