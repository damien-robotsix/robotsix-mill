...
Guard 1: review feedback is injected into the implement context on
EVERY spawn — including blocked-resume re-spawns.
Guard 2: a review re-spawn whose previous attempt committed only
...
# --- Guard 1: feedback injected on blocked-resume re-spawns --------------


def test_feedback_injected_on_blocked_resume(ctx_factory, tmp_path):
    """_load_implement_context includes review feedback even when the
    ticket carries ``blocked_from`` (a BLOCKED→READY resume)."""
    remote = make_bare_repo(tmp_path)
    ctx = ctx_factory(FORGE_REMOTE_URL=remote, test_command="true")
    t = _ticket(ctx)
    ctx.service.add_comment(t.id, "please fix the LICENSE check", author="reviewer")

    # Simulate a blocked-resume re-spawn: blocked_from recorded.
    t = ctx.service.get(t.id)
    t.blocked_from = State.READY.value
...
# --- Guard 2: changelog-only review re-spawn short-circuit ---------------
