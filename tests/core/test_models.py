...
def test_actiontype_all_values_are_lowercase_strings():
    """Every ActionType member's .value is a lowercase str."""
...
def test_actiontype_members_are_str_instances():
    """Every ActionType member isinstance(m, str) holds."""
    for member in ActionType:
        assert isinstance(member, str)
...
def test_proposedactionstatus_all_values_are_lowercase_strings():
    """Every ProposedActionStatus member's .value is a lowercase str."""
    for member in ProposedActionStatus:
        assert isinstance(member, str)
        assert isinstance(member.value, str)
        assert member.value.islower()


def test_proposedactionstatus_members_are_str_instances():
    """Every ProposedActionStatus member isinstance(m, str) holds."""
    for member in ProposedActionStatus:
        assert isinstance(member, str)


def test_proposedactionstatus_literal_values():
    """ProposedActionStatus members have the exact values specified in the spec."""
    assert ProposedActionStatus.PENDING.value == "pending"
...
def test_proposed_action_required_action_type():
    """action_type is required — omitting it raises ValidationError on validate."""
    with pytest.raises(ValidationError):
        ProposedAction.model_validate(
            ProposedAction(
                source="health",
...
def test_proposed_action_required_rationale():
    """rationale is required — omitting it raises ValidationError on validate."""
    with pytest.raises(ValidationError):
        ProposedAction.model_validate(
            ProposedAction(
                source="health",
...
def test_proposed_action_model_dump_and_validate_roundtrip():
...
# ---------------------------------------------------------------------------
# ProposedAction DB round-trip smoke test (uses the 'settings' fixture)
# ---------------------------------------------------------------------------


def test_proposed_action_db_roundtrip(settings):
    """Insert a PENDING ProposedAction row, commit, read back, and assert
    every field matches — including nullable fields surviving None."""
